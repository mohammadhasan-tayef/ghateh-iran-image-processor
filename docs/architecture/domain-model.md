...
StorageRoot references a server-known config key, never a client path. Accepted SourceObservation identity and provenance are immutable after hash completion. References in this document to current availability information do not decide whether availability belongs to SourceObservation or classify availability as accepted, operational, historical, or current state. Storage catalog may observe or supply current availability information or technical evidence; doing so does not establish semantic/domain ownership, direct mutation authority over SourceObservation, or permission to mutate SourceObservation-owned state. Mutation authority for availability, its persistence model, acceptance rules, lifecycle semantics, and representation remain deferred architectural decisions. Current availability information does not redefine accepted SourceObservation identity or provenance. A changed source creates a new observation. Preview artifacts are derived, independently replaceable by generation version, and cannot be treated as originals.
