...
| `Batch` | Ingestion, processing, and review-resolution boundary | id, root/source prefix, preset revision and snapshot, state, scan cursor, derived counts |
...
| `CandidateVersion` | Immutable artifact from a successful run | UUID id, run/batch-image ids, human-readable version no, image/mask keys/checksums, QC/provenance |
...
| `SubjectMode` | Fixed subject-selection policy value | single object, packaging, multi-component, all foreground, or manual selection required |
...
| `ProcessingEvent` | Append-only audit/diagnostic fact | id, subject, type, actor/task, correlation id, schema/payload |
...
`ProcessingRun` and `CandidateVersion` remain separate: failed/cancelled attempts produce no candidate; one successful run produces exactly one immutable candidate. Candidate UUID is identity. `version_no` is display ordering scoped to BatchImage and is allocated under a short BatchImage row lock during finalization.
...
- A Batch has many BatchImages and snapshots one immutable preset revision including SubjectMode.
- A BatchImage has many ProcessingRuns/CandidateVersions/ReviewDecisions and at most one selected CandidateVersion from itself.
...
Owns Batch scan progress, not all members in memory. Its terminal roll-up is independent of export. When all non-cancelled members are review-resolved, the batch becomes `review_completed` if none failed/cancelled, otherwise `partially_completed`. Rejection is a valid review resolution, not a failure. Durable counters are derived/reconciled.
...
One run idempotency key identifies one intended attempt. Candidate finalization locks BatchImage briefly, checks whether the run already has a candidate, allocates `next version_no` inside the transaction, inserts the UUID candidate, marks the run succeeded, and transitions the BatchImage. Concurrent reprocessing attempts may run, but each finalizer serializes allocation; late/stale lease completion is rejected. Gaps in version numbers are acceptable; collisions are not.
...
- CandidateVersion has `UNIQUE (id, batch_image_id)` and `UNIQUE (processing_run_id, batch_image_id)`.
...
Approval eligibility is additionally enforced transactionally by a review decision and selected candidate check; a relational FK alone cannot express effective human approval.
...
JSONB is allowed for validated preset snapshots, naming-policy snapshots, minimal session client metadata, QC facts/warnings, observed EXIF subsets, capabilities, scan cursors, error details, and event context. SubjectMode, role, identity/state, ownership, relationships, checksums, timestamps, routing fields, and expiry fields are typed columns. Every JSONB document is schema-versioned and size-bounded.
